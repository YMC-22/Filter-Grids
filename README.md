...
[![Version](https://img.shields.io/badge/version-3.1.0-blue.svg)](UPGRADE-NOTICE.md)
...
Usage Example:
```php
add_filter('ymc/post/layout/before/post_item_72', function ($post_number, $post_id, $paged, $per_page) {
   if($post_number === 2) {
     echo "<div>Insert custom HTML here. Post Number: {$post_number}. Post ID: {$post_id}. Paged: {$paged}. Per page: {$per_page}</div>";
   }
});
```
...
v1.2.0 – Added ymc/filter/layout/{placement}/custom_{id} filter

v1.1.0 – Initial hook support
